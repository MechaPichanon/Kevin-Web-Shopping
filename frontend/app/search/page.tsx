...
function SearchResults() {
  ...
  // Narrow by category + sub-category + price (client-side)
        ...
        <aside className="w-full flex-none rounded-2xl border border-black/10 bg-[#ece2d6] p-5 shadow-sm lg:sticky lg:top-24 lg:w-64">
          ...
          <div className="mb-2 flex flex-col gap-1">
            ...
            {visibleCategories.map((cat) => {
              ...
              const visibleSubCategories = cat.sub_categories.filter((sub) =>
                products.some((p) => p.sub_category === sub.sub_category)
              );
              const showSubs = active && visibleSubCategories.length > 0;
              return (
