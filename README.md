...
1. Start everything:
   - Make sure Ollama is running on your PC and listening on `http://localhost:11434`
   - One-time: pull models on your PC:
...
Notes:
- Docker is configured to use Ollama from your PC via `http://host.docker.internal:11434` (see `docker-compose.yml`).
...
- `OLLAMA_EMBED_MODEL` (default `nomic-embed-text`, recommended `bge-m3`)
- `RAG_TOP_K` (default `3`)
